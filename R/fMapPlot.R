...
fMapPlot <- 
  function(
    value=NULL,
    value_lim=NULL,
    value_order=c("Östergötland","Blekinge","Dalarna","Gävleborg","Gotland","Halland","Jämtland","Jönköping","Kalmar","Kronoberg","Norrbotten","Orebro","Södermanland","Skåne","Stockholm","Uppsala","Värmland","Västerbotten","Västernorrland","Västmanland","Västra Götaland"),
    value_order_return=FALSE,
    legend="",
    col=NULL,
    col_border="#7f7f7f",
    text_cex=1,
    ...
    if (value_order_return) {
      ...
      # Color
      if (is.null(col)) col <- "#FF9933"
      
      temp_value <- value
      temp_value[is.na(value)] <- 0
      
      if (is.null(value_lim)) value_lim <- range(pretty(c(min(0,min(value,na.rm=TRUE)),max(1,max(value,na.rm=TRUE)))),na.rm=TRUE)
        
      value_col <- 
        lightenCol(
          col=col,
          factor=(temp_value-value_lim[1])/(value_lim[2]-value_lim[1])
        )
      ...
      par(
        mai=c(
          0*linchheight,
          4*linchheight,
          (4*(!is.null(title))+2*(!is.null(title) & !is.null(subtitle1))+2*(!is.null(title) & !is.null(subtitle1) & !is.null(subtitle2)))*linchheight,
          1*linchheight
        ),
        bg="#ffffff",
        xpd=TRUE
      ...
      plot(
        gadm[gadm$NAME_2%in%"Heby",],
        col=value_col[value_names=="Uppsala"],
        border=value_col[value_names=="Uppsala"],
        add=TRUE
      )
      ...
      match <- coordinates_vastmanland[,1]<=16.85 & coordinates_vastmanland[,1]>=16.8 & coordinates_vastmanland[,2]<=temp_y_border & coordinates_vastmanland[,2]>=temp_y_border-0.03
      lines(coordinates_vastmanland[match,1],coordinates_vastmanland[match,2],lwd=1,col=col_border)
      
      plot(
        gadm[gadm$NAME_1%in%"Gävleborg",],
        col=value_col[value_names=="Gävleborg"],
        border=col_border,
        add=TRUE
      )
