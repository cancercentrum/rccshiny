...
fLinePlot <- 
  function(
    ...
    if (is.null(target_values_high)) target_values_high <- TRUE
    ...
    # Colors
    if (is.null(col)) {
      col <- c("#438DCC","#FFCC33","#EC6698","#003366","#339966","#66CCCC","#FF9933","#8A4F71","#9999CC","#E94949",
    ...
    par(
      mai=c(
        ifelse(!is.null(x_lab),6,4)*linchheight+ifelse(legend_pos=="bottom" & !is.null(legend),legend_nrow*linchheight,0),
        6*linchheight,
    ...
    if (!is.null(target_values) & length(target_values)>1) {
        ...
        rect(xleft=x_lim[1],ybottom=ifelse(target_values_high,max(target_values),y_lim[1]),xright=x_lim[2],ytop=ifelse(target_values_high,y_lim[2],min(target_values)),col=col_target_2,border=NA)
    }
    ...
    if (!is.null(legend)) {
      if (legend_pos=="bottom") {
        legend(x=x_lim[1]+0.5*(x_lim[2]-x_lim[1]),y=pos0y,legend=legend,col=col,pch=legend_pch,pt.cex=1.75,bty="n",cex=0.8*text_cex,xjust=0.5,yjust=0,y.intersp=1,ncol=legend_ncol,text.width=ifelse(!is.null(legend_textwidth),strwidth(paste(rep("X",legend_textwidth),collapse="")),max(strwidth(legend))))
      } else if (legend_pos=="right") {
        legend(x=x_lim[2]+0.1*(x_lim[1]-pos0x),y=0.5*sum(y_lim),legend=legend,col=col,pch=legend_pch,pt.cex=1.75,bty="n",cex=0.8*text_cex,yjust=0.5,y.intersp=2)
      }
    }
