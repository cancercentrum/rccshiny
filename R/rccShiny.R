rccShiny <- function(data = NULL, outcome = "outcome", outcomeTitle = outcome, folder = "ind", folderLinkText = outcomeTitle, titleTextBeforeSubtitle = NULL, titleTextAfterSubtitle = NULL,
    comment = "", description = c("(beskrivning saknas)", "(description missing)"), geoUnitsHospital = "sjukhus", geoUnitsCounty = "landsting", geoUnitsRegion = "region", geoUnitsFromLKF = FALSE,
    regionSelection = TRUE, regionLabel = c("Begränsa till region", "Limit to region"), period = "period", periodLabel = c("Diagnosår", "Year of diagnosis"), userInputList = NULL,
    targetValues = NULL, targetValuesSortDescending = NULL, hideLessThan = 5, language = c("sv"), npcrGroupPrivateOthers = TRUE) {
    ...
    for (loop_language in language) {
        ...
        save(GLOBAL_data, GLOBAL_outcome, GLOBAL_outcomeTitle, GLOBAL_outcomeClass, GLOBAL_titleTextBeforeSubtitle, GLOBAL_titleTextAfterSubtitle, GLOBAL_comment, GLOBAL_description,
            ...
            GLOBAL_targetValues, GLOBAL_targetValuesSortDescending, GLOBAL_userInputList, GLOBAL_hideLessThan, GLOBAL_language, GLOBAL_npcrGroupPrivateOthers, file = paste0("./apps/",
                loop_language, "/", folder, "/data/data.RData"))
