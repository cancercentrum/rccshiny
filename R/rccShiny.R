...
#' @param targetValues optional vector or list of vectors (one for each outcome) with 1-2 target levels to be plotted in the tabs Jämförelse/Comparison and Trend. If the outcome is numeric the target levels are only shown when "Andel inom..."/"Proportion within..." is selected, and then only for the default propWithinValue.
...
#' @param propWithinValue optional vector with default value(s) shown for numeric outcome(s) when "Andel inom..."/"Proportion within..." is selected. If the length of the vector is less than the number of numeric outcomes the values are recycled. Default is 30.
