...
fIndPlot <- 
  function(
    ...
    ind_numeric_percentiles=c(0.25,0.50,0.75),
    ...
    col=NULL,
    border=TRUE,
    x_max=if(ind_numeric){NULL}else{100},
    x_by=NULL,
    title=NULL,
    subtitle1=NULL,
    subtitle2=NULL,
    ...
    if (is.null(target_values_high)) target_values_high <- ifelse(ind_numeric,FALSE,TRUE)
    ...
    if (ind_numeric) {
      ind <- as.numeric(ind)
      if (ind_numeric_exclude_neg) include <- include & ind>=0
    }
        ...
        if (ind_numeric) {
          ...
          if (hide) {
            ...
            if (ind_factor_shownN) {
              measurements <- c(measurements,sum(measurements),if(!is.null(ind_factor_hide)){paste0(sum(x$ind%in%factor_legend)," av ",sum(!is.na(x$ind),na.rm=TRUE))}else{sum(!is.na(x$ind),na.rm=TRUE)})
            } else {
    ...
    if (ind_type=="factor") {
      tab$ind_sort <- round(rowSums(apply(cbind(tab[,if(!is.null(ind_factor_sortbycols)){paste0("factor",1:ind_factor_sortbycols)}else{substr(names(tab),1,6)=="factor"}]),2,as.numeric)),6)
    } else if (funnelplot) {
    ...
    col_factors <- c("#438DCC","#FFCC33","#EC6698","#003366","#339966","#66CCCC","#FF9933","#8A4F71","#9999CC","#E94949",
    ...
    par(
      mai=c(
        ifelse(!is.null(x_lab),6,4)*linchheight+legend_nrow*(ind_type=="factor")*linchheight+(num_periods>1)*linchheight+(ind_type=="factor" & num_periods>1)*linchheight+(funnelplot)*linchheight,
        ifelse(ind_numeric,linch_label+linch_n+linch_i,linch_label+linch_n),
        (2+2*(!is.null(title))+2*(!is.null(title) & !is.null(subtitle1))+2*(!is.null(title) & !is.null(subtitle1) & !is.null(subtitle2)))*linchheight,
    ...
    if (!is.null(target_values) & length(target_values)>1) {
      ...
      rect(xleft=ifelse(target_values_high,max(target_values),0),ybottom=0,xright=ifelse(target_values_high,x_max,min(target_values)),ytop=y_lim[2],col=col_target_2,border=NA)
    }
    ...
    if (!is.null(title)) {
      if (!is.null(subtitle1)) {
        if (!is.null(subtitle2)) {
          ...
          text(x=pos0x,y=y_n_label+0.3*(pos1y-y_n_label)+2*1.8*strheight("","user",cex=text_cex),labels=title,pos=4,cex=1.5*text_cex,offset=1)
        } else {
    ...
    # Axis label
    y_xlab_zeropos <- ifelse(ind_type=="factor",ifelse(num_periods>1,pos0y+2*luserheight,pos0y),ifelse(num_periods>1 | funnelplot,pos0y+luserheight,pos0y)) + legend_nrow*(ind_type=="factor")*luserheight
    ...
    if (ind_numeric) {
      ...
      for (i in 1:num_periods) {
        plotCI( x=rev(tab_list[[i]]$ind) , y=y_bp+(num_periods-i)*barheight , li=rev(tab_list[[i]]$lower) , ui=rev(tab_list[[i]]$upper) , col=rev(tab_list[[i]]$col) , pt.bg=par("bg") , lwd=2.5*point_cex , add=TRUE , pch=21 , cex=point_cex , err="x" , sfrac=0 , gap=0.75*point_cex/2.25 )
      }
    } else if (ind_type=="factor") {
      for (i in 1:num_periods) {
        for (j in rev(1:nrow(tab_list[[i]]))) {
          ...
          for (k in 2:length(temp_tab)) {
            rect(xleft=temp_tab[k-1],xright=temp_tab[k],ybottom=y_bp[length(y_bp)+1-j]-0.5*barheight+(num_periods-i)*0.5*barheight,ytop=y_bp[length(y_bp)+1-j]+0.5*barheight+(num_periods-i)*0.5*barheight,col=temp_col[k-1],border=ifelse(i==num_periods & tab_list[[i]]$group[j]%in%emph_lab,"#000000",ifelse(border,"#7f7f7f",NA)),lwd=ifelse(i==num_periods & tab_list[[i]]$group[j]%in%emph_lab,2,1))
          }
        }
      }
      ...
      # Legend
      legend(x=0.5*x_max,y=ifelse(num_periods>1,pos0y+2*luserheight,pos0y),legend=factor_legend,col=col,pch=15,pt.cex=1.75,bty="n",cex=0.8*text_cex,xjust=0.5,yjust=0,y.intersp=1,ncol=legend_ncol,text.width=if(legend_fixedtextwidth){max(strwidth(factor_legend))}else{NULL})
    } else {
      # Funnelplot
      if (funnelplot) {
        ...
        for (i in 1:length(funnelplot_probs)) {
          ...
          rect(xleft=temp_funnelplot_prev_lo,xright=temp_funnelplot_plot_lo,ybottom=y_bp-0.5*temp_funnelplot_sectionheight,ytop=y_bp+0.5*temp_funnelplot_sectionheight,col=temp_funnelplot_col[i],border=NA)
          rect(xleft=temp_funnelplot_plot_hi,xright=temp_funnelplot_prev_hi,ybottom=y_bp-0.5*temp_funnelplot_sectionheight,ytop=y_bp+0.5*temp_funnelplot_sectionheight,col=temp_funnelplot_col[i],border=NA)
          temp_funnelplot_prev_lo <- temp_funnelplot_plot_lo
          temp_funnelplot_prev_hi <- temp_funnelplot_plot_hi
        }
        # Funnelplot legend
        legend(x=0.5*x_max,y=pos0y,legend=paste0("p < ",funnelplot_probs),col=temp_funnelplot_col,pch=15,pt.cex=1.75,bty="n",cex=0.8*text_cex,xjust=0.5,yjust=0,y.intersp=1,ncol=length(funnelplot_probs))
      }
    ...
    if (num_periods>1) {
      ...
      legend(x=0.5*x_max,y=pos0y,legend=paste0(c(rep("",length(show_periods)-1),"*"),show_periods),col=period_legend_col,pch=15,pt.cex=1.75,bty="n",cex=0.8*text_cex,xjust=0.5,yjust=0,y.intersp=1,ncol=num_periods,text.width=max(strwidth(show_periods)))
    }
