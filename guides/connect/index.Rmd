...
Att publicera en applikation baserad på rccShiny kan ta ganska lång tid, räkna med ca 2 minuter per applikation. Det steg som tar lång tid är inte själva uppladdningen till servern, utan steget innan där applikationens paketberoenden ska bestämmas. Det verkar inte finnas någon bra lösning på detta i dagsläget, men det är ett känt problem och vi hoppas på en förbättring i kommande versioner av *rsconnect*.
...
För att göra API-anropen direkt från RStudio används paketen [httr](https://cran.r-project.org/web/packages/httr/index.html) och [connectapi](https://github.com/rstudio/connectapi). Det senare av dessa två finns inte på CRAN utan installeras enklast från GitHub via anropet:
```{r eval=FALSE}
